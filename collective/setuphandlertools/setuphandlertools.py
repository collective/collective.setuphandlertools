...
#       dict(
#           email=email,
#           fullname=fullname).update(data))  # <-- TODO: binary data doesn't
#                                             # work. mabe this string doesn't
#                                             # even work. have to check.
...
def update_portlet_schema(context, portlet_interface, attribute, value,
    ...
    for content in all_content:
        for manager_name, manager in\
                getUtilitiesFor(IPortletManager, context=content):
            ...
            for id, assignment in mapping.items():
                if portlet_interface.providedBy(assignment):
                    try:
                        ...
                        logger.info(
                            "attribute %s on portlet already set" % attribute)

                    except AttributeError:
                        setattr(assignment, attribute, value)
                        logger.info(
                            "attribute %s on portlet set with value %s "
                            % (attribute, value))
